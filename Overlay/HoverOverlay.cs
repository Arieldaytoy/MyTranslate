...
/// 悬停翻译浮窗 — 在鼠标旁边显示翻译结果
/// </summary>
public class HoverOverlay : OverlayForm
{
    private readonly Label _headerLabel;     // 顶部：语言方向 + 翻译来源
    ...
    public void ShowTranslation(TranslationResult result)
    {
        if (result.Success)
        {
            ...
            _contentLabel.ForeColor = result.IsCached
                ...
                : Color.FromArgb(30, 30, 30);   // API 翻译用黑色
        ...
        else
        {
            ...
            _retranslateButton.Visible = true; // 失败时也显示重翻按钮
        }

        AdjustSize();
    }

    /// <summary>获取当前翻译结果文本（供复制用）</summary>
